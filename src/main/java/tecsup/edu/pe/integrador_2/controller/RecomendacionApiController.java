...
public class RecomendacionApiController {
    ...
    public ResponseEntity<?> listarPorCultivo(
        ...
        List<Recomendacion> recomendaciones = recomendacionRepository.findByCultivoIdAndEstado(cultivoId, Estado.Activo);
        return ResponseEntity.ok(recomendaciones);
    }

    @PutMapping("/cultivo/{cultivoId}/eliminar")
    public ResponseEntity<?> eliminarRecomendacionesPorCultivo(
            @PathVariable Long cultivoId,
            @AuthenticationPrincipal OAuth2User principal) {
        ...
        for (Recomendacion rec : recomendaciones) {
            rec.setEstado(Estado.Inactivo);
        }
        recomendacionRepository.saveAll(recomendaciones);

        return ResponseEntity.ok("Todas las recomendaciones fueron marcadas como eliminadas");
    }
}